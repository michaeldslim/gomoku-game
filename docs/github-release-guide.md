...
- **앱 버전:** `app.json` → `expo.version` (현재 `2.1.1`)
...
| 항목 | 설명 |
|------|------|
| **태그** | `v2.1.1` 같은 버전 식별자 (보통 `v` + 시맨틱 버전) |
...
```
https://github.com/michaeldslim/gomoku-game/releases/tag/v2.1.1
```
...
| 2 | [버전 번호 맞추기](#3-버전-번호-맞추기) (`app.json`, `package.json`, `build.gradle`) | 예: `2.1.1` |
...
| 6 | 업로드용 파일명 정리 (권장) | `Gomoku-2.1.1-android.apk` 등 |
| 7 | Release 노트 초안 작성 | 변경 사항, 설치 방법 |

**Release 페이지에 가져갈 파일 (최소):**

- `Gomoku-2.1.1-android.apk` — Android 직접 설치용 (**필수**)

**선택 첨부:**

- `Gomoku-2.1.1-android.aab` — Play Store용 보관
- `Gomoku-2.1.1-ios.ipa` — Ad Hoc 테스터용
...
| 입력 | 예시 | 설명 |
|------|------|------|
| 태그 이름 | `v2.1.1` | `v` + `app.json`의 `expo.version` 권장 |

- **처음 릴리스:** 태그 이름 입력 → **`Create new tag: v2.1.1 on publish`** 선택
- **이미 태그가 있음:** 드롭다운에서 기존 `v2.1.1` 선택
...
| 예전 버전(`v2.0.0`)을 나중에 Publish | `v2.1.1`이 더 높으면 Latest는 `v2.1.1` 유지 |
...
gh release edit v2.1.1 --latest

# Latest에서 제외
gh release edit v2.1.1 --latest=false
```
...
- Release URL: `https://github.com/michaeldslim/gomoku-game/releases/tag/v2.1.1`
- **Code → Tags**에서 `v2.1.1` 태그 생성 여부 확인
...
```
[ ] 1. Choose a tag     → v2.1.1  (Create new tag on publish)
[ ] 2. Target           → main
[ ] 3. Release title    → Gomoku v2.1.1
[ ] 4. Description      → 변경 사항 + Android 설치 방법
[ ] 5. Attach binaries  → Gomoku-2.1.1-android.apk (+ 선택 AAB/IPA)
...
# 기존 릴리스에 파일 추가
gh release upload v2.1.1 ./Gomoku-app-release.aab ./Gomoku.ipa
```
...
| 플랫폼 | 파일 | 설치 방법 |
|--------|------|-----------|
| Android | `Gomoku-2.1.1-android.apk` | APK 다운로드 → 설치 (알 수 없는 출처 허용) |
| Android (Play) | `Gomoku-2.1.1.aab` | 개발자용 / Play Console 업로드 |
...
```
Gomoku-2.1.1-android.apk
Gomoku-2.1.1-android.aab
Gomoku-2.1.1-ios.ipa
```
...
- Draft release인지 확인 (`gh release view v2.1.1`)
...
*마지막 업데이트: 2026-07-10 · gomoku-game v2.1.1 기준*
